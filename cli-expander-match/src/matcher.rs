...
use cli_expander_config::{FieldConfig, MatchFile, VariableDef};
...
impl Matcher {
    ...
    pub fn from_files(files: Vec<(std::path::PathBuf, cli_expander_config::Config)>) -> Self {
...
fn find_trigger_at_word_boundary(input: &str, trigger: &str) -> Option<usize> {
    ...
    while let Some(pos) = input[start..].find(trigger) {
        ...
        let has_boundary_before =
            abs_pos == 0 || input.as_bytes()[abs_pos - 1].is_ascii_whitespace();
        let end = abs_pos + trigger.len();
        let has_boundary_after = end >= input.len()
            || input.as_bytes().get(end).is_none_or(|&b| {
